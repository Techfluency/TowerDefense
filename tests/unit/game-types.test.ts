...
describe('GAME_EVENTS', () => {
  ...
  it('should have exactly 31 events defined', () => {
    /* 12 original + 6 BOLT-001 + 1 BOLT-002 + 1 BOLT-003 (ENEMY_SPAWNED) + 1 BOLT-007 (TOWER_REPAIRED) + 1 BOLT-011 (AUTO_TILE_READY) + 2 BOLT-017 (ENEMY_SHIELD_BROKEN, ENEMY_SHIELD_REGENERATED) + 1 BOLT-019 (TOWER_BRANCH_SELECTED) + 3 BOLT-018 (BOSS_MINION_SUMMON, BOSS_SPEED_SURGE, BOSS_DIED) + 1 BOLT-020 (CAMPAIGN_COMPLETE) + 2 BOLT-022 (TOUCH_LONG_PRESS, TOUCH_DOUBLE_TAP). */
    expect(Object.keys(GAME_EVENTS).length).toBe(31);
  });

  it('should define BOLT-019 tower branch selected event', () => {
    expect(GAME_EVENTS.TOWER_BRANCH_SELECTED).toBe('TOWER_BRANCH_SELECTED');
  });
